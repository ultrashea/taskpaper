; TODO:
; Define function to focus on a tag
...
(defun taskpaper-functions()
  ...
  (defun taskpaper-archive-current-task()
    "Move the current task to Archive project"
    (interactive)
    (save-excursion
      (kill-whole-line 1)
      ...
      (forward-line -1)
      (beginning-of-line)
      ; Remove all but one tab at beginning of current line
      (replace-regexp "\t+" "\t")
    )
  )
  
  (defun taskpaper-archive-all-tasks()
    (interactive)
    (save-excursion
      (flush-lines "@done")
      (search-forward-regexp "^Archive:$")
...
; Remove multiple tabs
  ...
  (defun taskpaper-due-today()
    "Add due tag with today's date"
    (interactive)
    (let ((startpoint (point)) (line (line-number-at-pos)))
      (back-to-indentation)
      ;(re-search-forward "@done" nil 2)
